...
file0='2w_2w_2_n_batch'+str(args.n_batch)+'_batch_size_'+str(args.batch_size)+'sample_depth_'+str(args.sample_depth)+'_nheads_'+str(args.n_heads)+'_nlayers_'+str(args.n_layers)+'_sample_width_'+str(args.sample_width)+'_lr_'+str(args.lr)+'_n_hid_'+str(args.n_hid)
print(file0)
...
print(encoded)
print(encoded2)	
gnn = GNN(conv_name = 'hgt', in_dim = args.in_dim , \
          n_hid = args.n_hid, n_heads = args.n_heads, n_layers = args.n_layers, dropout = args.dropout,\
          num_types = 2, num_relations = 2).to(device)
...
device = torch.device("cpu")
model = GNN(conv_name = 'hgt', in_dim = args.in_dim ,n_hid = args.n_hid, n_heads = args.n_heads, n_layers = args.n_layers, dropout = args.dropout,\
             num_types = 2, num_relations = 2).to(device)
model.eval()
...
with  torch.no_grad():
    for i in range(1,int(upper)):
        if  i != int(gene_cell.shape[0]/ba+1):
            batch_gene = node_feature[(i-1)*ba:ba*i,]
        ...
        edge2 = list(c)	    
        edge_index = torch.tensor([edge1, edge2], dtype=torch.long)
        x={'gene': torch.tensor(node_feature[(i-1)*ba:ba*i,:], dtype=torch.float),
	                'cell': torch.tensor(encoded2, dtype=torch.float),
			        }
        edge_index_dict={('gene','g_c','cell'): torch.tensor([g, c], dtype=torch.long)}
        ...
        edge_type = torch.LongTensor(edge_type)
        node_rep = model.forward((torch.cat((batch_gene,node_feature[(gene_cell.shape[0]):(node_feature.shape[0]+1),:]),0)).to(device), node_type.to(device), \
                                             edge_time.to(device), edge_index.to(device), edge_type.to(device))
        gene_matrix = node_rep[node_type==0,]
        cell_matrix = node_rep[node_type==1,]
