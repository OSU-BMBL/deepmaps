...
parser.add_argument(
    "--factor", type=float, default="0.5", help="the attenuation factor"
)
parser.add_argument("--patience", type=int, default=5, help="patience")
parser.add_argument(
    "--rf", type=float, default="0.0", help="the weights of regularization"
)
parser.add_argument("--cuda", type=int, default=0, help="cuda 0 use GPU0 else cpu ")
...
def sub_sample(
    samp_nodes, sampled_depth=args.sample_depth, sampled_number=args.sample_width
):
    inp = {
...
for epoch in np.arange(args.epoch):
    ...
    for job in jobs:
        ...
        for t in types:
            ...
            if args.reduction != "raw":
                ...
                node_feature[t_i] = torch.tensor(feature[t], dtype=torch.float32).to(
                    device
                )
            ...
            if args.reduction == "raw":
                if epoch % 2 == 0:
                    loss = (
                        ...
                        + args.rf * regularization_loss
                    )
                else:
                    loss = (
                        nn.MSELoss()(node_feature[0], node_decoded_embedding[0])
                        + args.rf * regularization_loss
                    )
                    # print("loss=\n")
                    # print(loss)
                    for t_i in range(1, len(types)):
                        loss += nn.MSELoss()(
                            node_feature[t_i], node_decoded_embedding[t_i]
                        )
            else:
                loss = F.kl_div(
        ...
        if args.loss == "cross":
            ...
            torch.sigmoid(value)
            pos_loss = -torch.log(torch.sigmoid(value) + EPS).mean()
            pos_edge_index = edge_index

            pos_edge_index, _ = remove_self_loops(pos_edge_index)
            pos_edge_index, _ = add_self_loops(pos_edge_index)
            neg_edge_index = negative_sampling(pos_edge_index, node_rep.size(0))
            value = (node_rep[neg_edge_index[0]] * node_rep[neg_edge_index[1]]).sum(
                dim=1
            )
    ...
    print(
        ...
        epoch + 1,
        "|",
        "train_loss:%.12f" % (L / (int(gene_cell.shape[0])) / args.n_batch),
    )
...
with torch.no_grad():
    for i in range(0, gene_cell.shape[0], ba):
        ...
        x = {
            "gene": torch.tensor(encoded[i : (ba + i), :], dtype=torch.float),
            "cell": torch.tensor(encoded2, dtype=torch.float),
        ...
        if args.reduction != "raw":
            node_rep = gnn.forward(
                (torch.cat((x["gene"], x["cell"]), 0)).to(device),
                node_type.to(device),
                edge_time.to(device),
                edge_index.to(device),
                edge_type.to(device),
            ...
            node_rep, _ = gnn.forward(
                [x["gene"].to(device), x["cell"].to(device)],
                node_type.to(device),
                edge_time.to(device),
                edge_index.to(device),
                edge_type.to(device),
