...
file0='n_batch'+str(args.n_batch)+'_batch_size_'+str(args.batch_size)+'sample_depth_'+\
       ...
       +'_layertype_'+str(args.layer_type)+'_loss_'+str(args.loss)+'_optimizer_'+str(args.optimizer)
...
def load_data(path, sep, col_name, row_name):
    ...
    if row_name == True:
        ...
        gene_name= ''
    gene_cell = np.array(gene_cell)
    print("The number of gene is {}, The number of cell is {}".format(gene_cell.shape[0],gene_cell.shape[1]))
...
def sub_sample(samp_nodes,sampled_depth = args.sample_depth, sampled_number = args.sample_width ):
    inp = {'gene': np.concatenate([samp_nodes, graph.years[samp_nodes]]).reshape(2, -1).transpose()}
    '''
...
gene_cell, gene_name, cell_name = load_data(  args.input_dir,sep=" ",col_name = True, row_name = True)
...
for batch_id in np.arange(args.n_batch):
    ...
    p = sub_sample(np.random.choice(np.arange(gene_cell.shape[0]), args.batch_size, replace = False))
    jobs.append(p)
...
scheduler=torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min',factor=args.factor, patience=args.patience, verbose=True)
...
for epoch in np.arange(args.epoch):
    ...
    for job in jobs:
        ...
        for t in types:
            ...
            if args.reduction != 'raw':
                ...
                node_feature[t_i]=torch.tensor(feature[t],dtype=torch.float32).to(device)
        ...
        if (args.loss=="kl"):
            ...
            if args.reduction=='raw':
                if epoch%2==0:
                     loss = F.kl_div(decoder.softmax(dim=-1).log(), adj.softmax(dim=-1), reduction='sum')+args.rf*regularization_loss
                else:
                     loss = nn.MSELoss()(node_feature[0],node_decoded_embedding[0])+args.rf*regularization_loss
                     #print("loss=\n")
                     #print(loss)
                     for t_i in range(1,len(types)):
                          loss += nn.MSELoss()(node_feature[t_i],node_decoded_embedding[t_i])
            else:
                 loss = F.kl_div(decoder.softmax(dim=-1).log(), adj.softmax(dim=-1), reduction='sum')
        ...
        if (args.loss=="cross"):
            EPS = 1e-15 
            value = (node_rep[edge_index[0]] * node_rep[edge_index[1]]).sum(dim=1)
            torch.sigmoid(value)
            pos_loss = -torch.log(
            ...
            neg_edge_index =  negative_sampling(pos_edge_index, node_rep.size(0))
            value = (node_rep[neg_edge_index[0]] * node_rep[neg_edge_index[1]]).sum(dim=1)
    ...
    #print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    print('Epoch :', epoch+1,'|','train_loss:%.12f'%( L/(int(gene_cell.shape[0]))/args.n_batch))    
...
g=np.nonzero(gene_cell)[0]
c=np.nonzero(gene_cell)[1]+gene_cell.shape[0]
name1=pd.DataFrame(gene_name[0:torch.tensor([g, c]).shape[1]], columns=['gene'])
name2=pd.DataFrame(cell_name[0:torch.tensor([g, c]).shape[1]], columns=['cell']) 
